...
for x in range(3):
    area.append([])
    for y in range(3):
        button = tk.Button(window, text="", width=6, height=3,font=("Georgia", 20))
        area[x].append(button)
        area[x][y].place(x=x * 100, y=y * 100)
        area[x][y]["command"] = lambda selected_button = button: push(selected_button) # забираем с собой кнопку
# print(area)
window.mainloop()